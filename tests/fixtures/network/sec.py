...
def filter_aapl() -> Filter.Selectors:
    return Filter.Selectors(
        ...
        sec_filter=Filter.SecFilter(
            ...
            only_annual=False) # We want the 10-Q
    )

@pytest.fixture
def sec_dataselector_2023q1(sec_instance: Sec, filter_aapl: Filter.Selectors) -> DataSelector:
    return sec_instance.getData(tickers=filter_aapl.ticker_filter, filter=filter_aapl.sec_filter)
